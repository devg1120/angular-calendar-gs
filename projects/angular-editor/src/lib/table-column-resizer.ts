...
interface Style { gripid: any; styleSheet: any }
...
/*
    //attributes:
    resizeMode:any = 'fit';              //mode can be 'fit', 'flex' or 'overflow'
    draggingClass:any =  'grip-drag';	    //css-class used when a grip is being dragged (for visual feedback purposes)
    gripInnerHtml:any =  '';				//if it is required to use a custom grip it can be done using some custom HTML
    liveDrag:any =  false;				//enables table-layout updating while dragging
    minWidth:any =  15; 					//minimum width value in pixels allowed for a column
    headerOnly:any =  false;				//specifies that the size of the the column resizing anchors will be bounded to the size of the first row
    hoverCursor:any =  'col-resize';  	//cursor to be used on grip hover
    dragCursor:any =  'col-resize';  		//cursor to be used while dragging
    flush:any =  false; 					//when it is required to prevent layout restoration after postback, 'flush' will remove its associated layout data
    marginLeft:any =  null;				//e.g. '10%', '15em', '5px' ...
    marginRight:any =  null; 				//e.g. '10%', '15em', '5px' ...
    remoteTable:any =  null;              //other table element to resize using the main table as a controller
    disable:any =  false;					//disables all the enhancements performed in a previously resized table
    partialRefresh:any =  false;			//can be used when the table is inside of an updatePanel,
    disabledColumns:any =  [];            //column indexes to be excluded
    removePadding:any =  true;            //remove padding from the header cells.
    widths:any =  [];                     //array of initial column widths
    serialize:any =  true;
...
*/
    ...
    createStyle = (element, css) => {
        ...
        const filtered = Array.from(oldStyle).filter((style :any) => {
            return (style.gripid === hash);
        });
        if (filtered.length) {
            return;
        }
	    //let style: HTMLStyleElement| object = document.createElement('style');
    ...
    extendOptions = (options) => {
        ...
        const extOptions = Object.assign({}, this.DEFAULTS, options);
     ...
     */
    createDiv = (element, className, text = null) => {
